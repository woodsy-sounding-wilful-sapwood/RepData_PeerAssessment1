...
This individual took, on average, `r prettyNum(mean_steps, big.mark=",")` steps every day day. The median number of steps per day was `r prettyNum(median_steps, big.mark=",")`.
...
```{r}
  ...
  max_interval
```
The interval with the highest average step count is the interval starting at minute `r max_interval`.
...
```
Because we replaced `r NA` values with the mean for the interval across all days, the mean total step count per day was unaffected and remained `r prettyNum(mean_impute_steps, big.mark=",")`. However, the median was pulled towards the mean and the reduced to `r prettyNum(median_impute_steps, big.mark=",")`. If we had used the median per interval, the opposite would have happened. Note also that the median is no longer a Natural Number.

While the location of the histogram remained identical, its spread was reduced considerable. By replace `r NA` values with the mean, we have artificially reduced the variance of the distribution.
